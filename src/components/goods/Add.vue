  ...
  <div>
   <!-- 面包屑导航区域 -->
    ...
    <el-card>
      ...
      <!-- 左侧分页区域 -->
      <el-form :model="addForm" :rules="addFormRules" ref="addFormRef" label-width="100px" class="addForm" label-position="top">
        <el-tabs :tab-position="'left'" v-model="activeIndex" :before-leave="beforeTabLeave" @tab-click = "tabClick">
          ...
          <el-tab-pane label="商品属性" name="2">
            <el-form-item :label="item.attr_name" :key="item.attr_id" v-for="item in onlyTabData">
  ...
  data() {
    return {
      ...
      addFormRules: {
        ...
        goods_cat: [{ required: true, message: '请选择商品种类', trigger: 'blur' }]
      },
      //  商品分类数据
      cateList: [],
      //  级联选择器绑定对象
  ...
  methods: {
    ...
    async tabClick() {
      ...
      if (this.activeIndex === '1') {
        ...
        console.log('manyTabData:')
        console.log(this.manyTabData)
        // console.log(typeof this.manyTabData)
        ...
        console.log('onlyTabData:')
        console.log(this.onlyTabData)
        // console.log(typeof this.onlyTabData)
      }
    },
    //  处理图片预览效果
    handlePreview() {},
    ...
    add() {
      this.$refs.addFormRef.validate(async valid => {
        ...
        //  goods_cat应变为后端要求的以','分割的分类列表
        //  由于级联选择器与addform绑定，在这里改变addForm中的数据后，绑定数据不再适应级联选择器
        //  的绑定要求，所以对表单进行深拷贝，复制出一个完全一样的新对象，对新对象进行操作，以此来避免对之前对象的影响
